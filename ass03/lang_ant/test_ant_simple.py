# -*- coding: utf-8 -*-
"""
Game of life simple script for checking init states and checking if the evolution is
implemented correctly.

Created on Tue Jan 15 12:37:52 2019

@author: shakes
"""
import lang_ant
import reader

# f_name = input("Filename: ")
# init_array = reader.read_file(f_name)
# print(init_array)
#
# N = max(32, len(init_array))
N = 64
# create the game of life object
ant = lang_ant.Ant(20, 20, 'N')
lattice = lang_ant.LangAnt(ant, N)
# lattice.insert_from_file(init_array, (0, 0))

# N = 64

#create the game of life object
# life = conway.GameOfLife(N)
# life.insertBlinker((0,0))
#life.insertGlider((0,0))
# life.insertGliderGun((0,0))
cells = lattice.getStates() #initial state

#evolve once
lattice.step()
cellsUpdated1 = lattice.getStates()

#evolve twice
lattice.step()
cellsUpdated2 = lattice.getStates()

#evolve twice
lattice.step()
cellsUpdated2 = lattice.getStates()

#evolve twice
lattice.step()
cellsUpdated2 = lattice.getStates()

#evolve twice
lattice.step()
cellsUpdated2 = lattice.getStates()

#-------------------------------
#plot cells
import matplotlib.pyplot as plt
import numpy as np

plt.figure(0)
plt.gray()
plt.imshow(cells)

ax = plt.gca()
# Minor ticks
ax.set_xticks(np.arange(-.1, N, 1), minor=True);
ax.set_yticks(np.arange(-.1, N, 1), minor=True);
#grid
ax.grid(which='minor', color='w', linestyle='-', linewidth=1)

#plt.figure(1)
#plt.imshow(cellsUpdated1)
#
#plt.figure(2)
#plt.imshow(cellsUpdated2)

plt.show()
